...
"Plug 'nekowasabi/vim-annotation' "plug para anotaciones :AnnotationOpen :AnnotationDelete
...
let g:python3_host_prog = 'C:\Users\DAVIK\AppData\Local\Programs\Pytho\Python312\python.exe'
...
" NOTE: desde aqui es para las funcionalidades para coc las comente para
" windows ya que genera conflicto
...
" Función para gestionar el portapapeles según el entorno
function! CopyToClipboard()
  if has('win32') || has('win64')  " Verifica si estamos en Windows
    " Usamos clip.exe para copiar el contenido al portapapeles de Windows
    call system('clip.exe', @")
  elseif has('wsl')  " Verifica si estamos en WSL
    " Usamos clip.exe a través de WSL para copiar al portapapeles de Windows
    call system('powershell.exe -command "Set-Clipboard -Value \'' . substitute(@", "'", "''", "g") . '\'"')
  elseif executable('xclip')  " Verifica si estamos en un entorno Linux y si xclip está disponible
    " Usamos xclip para copiar al portapapeles en Linux
    call system('echo "' . substitute(@", '"', '\\"', "g") . '" | xclip -selection clipboard')
  elseif executable('xsel')  " Si xclip no está disponible, probamos con xsel
    " Usamos xsel para copiar al portapapeles en Linux
    call system('echo "' . substitute(@", '"', '\\"', "g") . '" | xsel --clipboard --input')
  else
    echom "No se ha detectado un comando para gestionar el portapapeles en este entorno."
  endif
endfunction

" Usar esta función cuando se haga un "yank" (copiado)
